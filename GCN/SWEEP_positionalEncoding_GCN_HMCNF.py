...
def compute_rwpe(edge_index, num_nodes, k=8):
    ...
    RW_power = RW.copy()
    for _ in range(k):
        rwpe.append(np.diag(RW_power))
        RW_power = RW_power @ RW
    
    return torch.tensor(np.stack(rwpe, axis=1), dtype=torch.float)
...
class SmellGATV2_HMCNF(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, num_heads = 1, num_classes=138, num_parents=12, beta=0.5, dims=dims):
        ...
        self.conv1 = GCNConv(num_node_features, c1)
        self.conv2 = GCNConv(c1, c2)
        self.conv3 = GCNConv(c2, c3)
        self.conv4 = GCNConv(c3, c4)
...
N_RUNS = 50
master_csv    = "random_seed_sweep_results_positionalEncoding_GCN.csv"
per_label_csv = "best_model_per_label_test_metrics_positionalEncoding_GCN.csv"
...
for i in range(N_RUNS):
    ...
    test_df, per_label_df = run_single_split_sweep(
        ...
        dropout=0.55,
        monitor="val_aucpr",
        results_dir=f"single_split_results_positionalEncoding_GCN_seed{SEED}",
    )
