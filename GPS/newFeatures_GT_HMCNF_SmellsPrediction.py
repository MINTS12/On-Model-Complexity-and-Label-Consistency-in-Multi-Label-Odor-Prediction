...
from rdkit.Chem import MolFromSmarts, rdPartialCharges
...
metrics_path   = "gps_hmcnf_metrics.csv"
batch_size     = 128
num_heads      = 8
lambda_hier    = 0.3
learning_rate  = 0.001
...
META_CATEGORIES = {
    "macro_floral": ["floral", "rose", "jasmin", "lily", "muguet", "violet", "hyacinth", "geranium", "lavender", "orangeflower", "chamomile", "hawthorn"],
    "macro_fruity": ["fruity", "apple", "apricot", "banana", "berry", "cherry", "grape", "grapefruit", "lemon", "melon", "orange", "peach", "pear", "pineapple", "plum", "raspberry", "strawberry", "tropical", "black currant", "fruit skin"],
    "macro_sweet": ["sweet", "vanilla", "caramellic", "honey", "chocolate", "cocoa", "coconut", "creamy", "buttery", "milky", "dairy"],
    "macro_woody": ["woody", "cedar", "sandalwood", "pine", "vetiver", "terpenic", "balsamic", "cortex"],
    "macro_green": ["green", "grassy", "herbal", "leafy", "hay", "tea", "fresh", "cucumber", "vegetable", "weedy"],
    "macro_spicy": ["spicy", "cinnamon", "clove", "warm", "pungent", "sharp", "cooling", "mint", "camphoreous"],
    "macro_animal_musk": ["animal", "musk", "leathery", "fishy", "sweaty", "meaty", "beefy", "musty"],
    "macro_earthy": ["earthy", "mushroom", "nutty", "hazelnut", "roasted", "coffee", "tobacco", "smoky", "popcorn"],
    "macro_citrus": ["citrus", "bergamot", "ozone", "clean", "soapy"],
    "macro_chemical": ["solvent", "ethereal", "metallic", "medicinal", "phenolic", "sulfurous", "gassy", "burnt", "oily"],
    "macro_gourmand": ["almond", "malty", "rummy", "brandy", "cognac", "winey", "cooked", "potato", "savory", "celery", "tomato", "radish", "onion", "garlic", "cabbage", "cheesy"],
    "macro_powdery_amber": ["amber", "powdery", "anisic", "coumarinic", "orris", "waxy", "aldehydic", "ketonic", "lactonic"],
}
...
def smiles_to_graph(smiles: str, y_tensor: torch.Tensor) -> Data | None:
    ...
    for atom in mol.GetAtoms():
        ...
        node_features.append([
            ...
            int(h not in (HybridizationType.SP, HybridizationType.SP2, HybridizationType.SP3)),
...
rwpe = AddRandomWalkPE(walk_length=WALK_LENGTH, attr_name='pe')
...
train_loader = DataLoader(train_data, batch_size=128, shuffle=True, num_workers=NUM_WORKERS, pin_memory=True, persistent_workers=PERSISTENT_WORKERS, generator=torch.Generator().manual_seed(SEED))
...
def _gine_mlp(in_dim: int, out_dim: int) -> nn.Module:
    return Sequential(
...
class SmellGPS_HMCNF(torch.nn.Module):
    def __init__(
        self,
        ...
        node_in_dim = num_node_features + walk_length
        self.node_emb = Sequential(
            Linear(node_in_dim, hidden_dim),
        ...
        for _ in range(num_layers):
            gine_conv = GINEConv(
                nn=_gine_mlp(hidden_dim, hidden_dim),
                edge_dim=edge_dim,        
                train_eps=True,
            )
            gps = GPSConv(
        ...
        self.mlp_input_dim = (num_layers + 1) * hidden_dim
 
        mid1 = 192
        mid2 = 128
 
        self.global_mlp1  = Linear(self.mlp_input_dim, mid1)
        self.global_bn1   = BatchNorm1d(mid1)
        self.global_drop1 = Dropout(dropout)
 
        self.global_mlp2  = Linear(mid1, mid2)
        self.global_bn2   = BatchNorm1d(mid2)
        self.global_drop2 = Dropout(dropout)
 
        self.global_out = Linear(mid2, num_classes)
 
        self.local1_transition = Linear(mid1, mid1 // 2)
        self.local1_bn         = BatchNorm1d(mid1 // 2)
        self.local1_out        = Linear(mid1 // 2, num_parents)
 
        self.local2_transition = Linear(mid2, mid2)
        self.local2_bn         = BatchNorm1d(mid2)
        self.local2_out        = Linear(mid2, num_classes)
    ...
    def forward(self, data):
        ...
        if self.walk_length > 0 and hasattr(data, 'pe'):
        ...
        for gps in self.gps_layers:
            x = gps(x, edge_index, batch, edge_attr=edge_attr)
            pooled.append(global_add_pool(x, batch))
 
        graph_repr = torch.cat(pooled, dim=1)
 
        h1 = F.relu(self.global_bn1(self.global_mlp1(graph_repr)))
        h1 = self.global_drop1(h1)
 
        h2 = F.relu(self.global_bn2(self.global_mlp2(h1)))
...
def run_sweep(
    ...
    fieldnames = [
        ...
        "beta", "dropout", "lr",
        "hidden_dim", "num_layers", "walk_length", "edge_dim",
    ...
    for run_idx, (lam, heads, t0, epochs_) in enumerate(grid):
        run_id  = f"run{run_idx:03d}_lam{lam}_h{heads}_t0{t0}_ep{epochs_}"
        ts_start = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"\n[{run_idx+1}/{len(grid)}] {run_id}  started {ts_start}")

        set_seed(SEED)

        model = SmellGPS_HMCNF(
        ...
        row = dict(
            ...
            lambda_hier=lam, num_heads=heads, T_0=t0, epochs=epochs_,
            beta=beta, dropout=dropout, lr=learning_rate,
            hidden_dim=hidden_dim, num_layers=num_layers,
            walk_length=walk_length, edge_dim=edge_dim,
...
def evaluate_sweep(
    ...
    for i, row in enumerate(sweep_rows):
        ...
        print(f"  → auroc_138 {auroc_138:.4f} | f1_macro_138 {out_row['test_f1_macro_138']} "
              f"| auroc_12 {auroc_12:.4f} | f1_macro_12 {out_row['test_f1_macro_12']}")
...
def evaluate_best_model_per_label(
    ...
    walk_length = int(best_row["walk_length"])
    edge_dim    = int(best_row["edge_dim"])
...
N_RUNS = 50
master_csv    = "random_seed_sweep_results_GT.csv"
per_label_csv = "best_model_per_label_test_metrics_GT.csv"
...
for i in range(N_RUNS):
    ...
    test_df, per_label_df = run_single_split_sweep(
        ...
        num_layers=3,
        monitor="val_aucpr",
        results_dir=f"single_split_results_GT_seed{SEED}",
    )
